...
public class CrackShot implements Listener {
    ...
    public void AnimalKilling(WeaponDamageEntityEvent event) {
        ...
        if (this.plugin.getNms().isAnimal(entity)) {
            if (res != null && res.getPermissions().playerHas(cause, Flags.animalkilling, FlagCombo.OnlyFalse)) {
                cause.sendMessage(this.plugin.msg(lm.Residence_FlagDeny, Flags.animalkilling.getName(), res.getName()));
    ...
    public void onEntityDamageByEntityEvent(WeaponDamageEntityEvent event) {
		...
		if (event.getVictim().getType() != EntityType.ITEM_FRAME && !this.plugin.getNms().isArmorStandEntity(event.getVictim().getType())) {
			return;
		}
    ...
    public void onEntityDamage(WeaponDamageEntityEvent event) {
        ...
        ClaimedResidence area = this.plugin.getResidenceManager().getByLoc(victim.getLocation());
        /* Living Entities */
        ...
        if (!srcpvp) {
            damager.sendMessage(this.plugin.msg(lm.General_NoPVPZone));
            event.setCancelled(true);
            return;
        }
        ...
        if (area == null) {
            /* World PvP */
            if (!this.plugin.getWorldFlags().getPerms(damager.getWorld().getName()).has(Flags.pvp, true)) {
                damager.sendMessage(this.plugin.msg(lm.General_WorldPVPDisabled));
                event.setCancelled(true);
            }
        } else {
            /* Normal PvP */
