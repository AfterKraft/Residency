...
//	    ConcurrentHashMap<String, List<ShopVote>> VoteList = plugin.getShopSignUtilManager().GetAllVoteList();
            ...
            if (!res.GetShopVotes().isEmpty()) {
                ...
                for (ShopVote OneVote : list) {
                    if (OneVote.getName().equalsIgnoreCase(player.getName()) || OneVote.getUuid() != null && OneVote.getUuid() == player
                        .getUniqueId()) {
                        if (plugin.getConfigManager().isOnlyLike()) {
    ...
    public void getLocale(ConfigReader c, String path) {
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "vote"), Arrays.asList("[residence]", "10"));
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "like"), Arrays.asList("[residenceshop]"));

        c.get(path + "votes.Description", "Shows res shop votes");
        c.get(path + "votes.Info",
            Arrays.asList("&eUsage: &6/res shop votes <residence> <page>", "Shows full vote list of current or defined residence shop"));
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "votes"), Arrays.asList("[residenceshop]"));

        c.get(path + "likes.Description", "Shows res shop likes");
        c.get(path + "likes.Info",
            Arrays.asList("&eUsage: &6/res shop likes <residence> <page>", "Shows full like list of current or defined residence shop"));
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "likes"), Arrays.asList("[residenceshop]"));

        c.get(path + "setdesc.Description", "Sets residence shop description");
        c.get(path + "setdesc.Info",
            Arrays.asList("&eUsage: &6/res shop setdesc [text]", "Sets residence shop description. Color code supported. For new line use /n"));
        ...
        c.get(path + "deleteboard.Info",
            Arrays.asList("&eUsage: &6/res shop deleteboard", "Deletes res shop board bi right clicking on one of signs"));
    }
}
