    ...
    public boolean perform(Residence plugin, String[] args, boolean resadmin, Command command, CommandSender sender) {
        ...
        if (args.length == 2) {
            ...
            if (args[1].equals("gset")) {
                plugin.getPermissionListManager().getList(player.getName(), args[2])
                    .setGroupFlag(args[3], args[4], FlagPermissions.stringToFlagState(args[5]));
                plugin.msg(player, lm.Flag_Set, args[3], args[2], FlagPermissions.stringToFlagState(args[4]));
                return true;
            } else if (args[1].equals("pset")) {
                plugin.getPermissionListManager().getList(player.getName(), args[2])
                    .setPlayerFlag(args[3], args[4], FlagPermissions.stringToFlagState(args[5]));
                plugin.msg(player, lm.Flag_Set, args[3], args[2], FlagPermissions.stringToFlagState(args[4]));
                return true;
            }
        }
