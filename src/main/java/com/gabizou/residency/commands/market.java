...
public class market implements cmd {
        ...
        switch (secondCommand.toLowerCase()) {
            ...
            case "sign":
                ...
                if (plugin.getSignUtil().getSigns().GetAllSigns().size() > 0) {
                    category =
                        plugin.getSignUtil().getSigns().GetAllSigns().get(plugin.getSignUtil().getSigns().GetAllSigns().size() - 1).GetCategory() + 1;
                }
    ...
    private boolean commandResMarketList(String[] args, Player player, int page) {
        ...
        this.plugin.msg(player, lm.General_MarketList);
        if (args.length < 3) {
            return false;
        }

        if (args[2].equalsIgnoreCase("sell")) {
            this.plugin.getTransactionManager().printForSaleResidences(player, page);
            return true;
        }
        if (args[2].equalsIgnoreCase("rent")) {
            if (this.plugin.getConfigManager().enabledRentSystem()) {
                this.plugin.getRentManager().printRentableResidences(player, page);
            }
            return true;
        }
        return false;
    ...
    private boolean commandResMarketAutoPay(String[] args, boolean resadmin, Player player) {
        ...
        if (args.length == 3) {
            barg = args[2];
            res = this.plugin.getResidenceManager().getByLoc(player.getLocation());
        } else {
            barg = args[3];
            res = this.plugin.getResidenceManager().getByName(args[2]);
        }
        ...
        if (barg.equalsIgnoreCase("true") || barg.equalsIgnoreCase("t")) {
            ...
            this.plugin.msg(player, lm.Invalid_Boolean);
            return true;
        }
        ...
        if (res.isRented() && res.getRentedLand().player.equalsIgnoreCase(player.getName())) {
            this.plugin.getRentManager().setRentedRepeatable(player, res.getName(), value, resadmin);
        } else if (res.isForRent()) {
            this.plugin.getRentManager().setRentRepeatable(player, res.getName(), value, resadmin);
        } else {
            this.plugin.msg(player, lm.Economy_RentReleaseInvalid, ChatColor.YELLOW + res.getName() + ChatColor.RED);
        }
        return true;
    }
    ...
    private boolean commandResMarketPayRent(String[] args, boolean resadmin, Player player) {
        ...
        if (res != null) {
            this.plugin.getRentManager().payRent(player, res, resadmin);
        } else {
            this.plugin.msg(player, lm.Invalid_Residence);
        }
        return true;
    ...
    private boolean commandResMarketRentable(String[] args, boolean resadmin, Player player) {
        ...
        try {
            ...
            this.plugin.msg(player, lm.Invalid_Cost);
            return true;
        }
        try {
            days = Integer.parseInt(args[4]);
        } catch (Exception ex) {
            this.plugin.msg(player, lm.Invalid_Days);
            return true;
        }
        boolean AllowRenewing = this.plugin.getConfigManager().isRentAllowRenewing();
        if (args.length >= 6) {
            String ag = args[5];
        ...
        boolean StayInMarket = this.plugin.getConfigManager().isRentStayInMarket();
        if (args.length >= 7) {
            String ag = args[6];
            if (ag.equalsIgnoreCase("t") || ag.equalsIgnoreCase("true")) {
                StayInMarket = true;
        ...
        boolean AllowAutoPay = this.plugin.getConfigManager().isRentAllowAutoPay();
        if (args.length >= 8) {
            String ag = args[7];
            if (ag.equalsIgnoreCase("t") || ag.equalsIgnoreCase("true")) {
                AllowAutoPay = true;
        ...
        this.plugin.getRentManager().setForRent(player, args[2], cost, days, AllowRenewing, StayInMarket, AllowAutoPay, resadmin);
        return true;
    }
    ...
    private boolean commandResMarketRent(String[] args, boolean resadmin, Player player) {
        ...
        boolean repeat = this.plugin.getConfigManager().isRentPlayerAutoPay();
        ...
        if (res != null) {
            this.plugin.getRentManager().rent(player, res, repeat, resadmin);
        } else {
            this.plugin.msg(player, lm.Invalid_Residence);
        }
    ...
    public void getLocale(ConfigReader c, String path) {
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "Info"), Arrays.asList("[residence]"));
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "list"), Arrays.asList("rent%%sell"));
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "sell"), Arrays.asList("[residence]"));
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "sign"), Arrays.asList("[residence]"));
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "buy"), Arrays.asList("[residence]"));
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "unsell"), Arrays.asList("[residence]"));
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "rent"), Arrays.asList("[cresidence]", "true%%false"));

        c.get(path + "rentable.Description", "Make a residence rentable.");
        c.get(path + "rentable.Info",
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "rentable"), Arrays.asList("[residence]", "1000", "7", "true", "true",
                "true"));
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "autopay"), Arrays.asList("[residence]%%true%%false", "true%%false"));
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "payrent"), Arrays.asList("[residence]"));
        ...
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "release"), Arrays.asList("[residence]"));
        Residence.getInstance().getLocaleManager().CommandTab
            .put(Arrays.asList(this.getClass().getSimpleName(), "unrent"), Arrays.asList("[residence]"));
    }
