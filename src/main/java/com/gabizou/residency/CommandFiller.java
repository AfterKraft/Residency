...
import com.gabizou.residency.containers.CommandStatus;
...
public class CommandFiller {
    ...
    public final String packagePath = "com.gabizou.residency.commands";
    ...
    public List<String> getCommands(Boolean simple) {
        ...
        for (Entry<String, CommandStatus> one : this.CommandList.entrySet()) {
    ...
    public Map<String, CommandStatus> fillCommands() {
        ...
        try {
            lm = getClassesFromPackage(this.packagePath);
        ...
        for (Entry<String, Class<?>> OneClass : classes.entrySet()) {
            ...
            for (Method met : OneClass.getValue().getMethods()) {
                ...
                this.CommandList.put(cmd, new CommandStatus(simple, Priority));
                break;
            }
            if (!found) {
                this.CommandList.put(OneClass.getKey(), new CommandStatus(true, 1000));
            }
        }
    ...
    private Class<?> getClass(String cmd) {
        Class<?> nmsClass = null;
        try {
            nmsClass = Class.forName(this.packagePath + "." + cmd.toLowerCase());
        } catch (ClassNotFoundException e) {
