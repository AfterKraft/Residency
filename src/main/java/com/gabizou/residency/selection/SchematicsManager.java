...
public class SchematicsManager {
    ...
    public boolean save(ClaimedResidence res) {
        ...
        dir = new File(this.plugin.getDataLocation(), "Schematics" + File.separator + res.getWorld());
        if (!dir.exists()) {
            try {
                dir.mkdir();
            } catch (SecurityException se) {
        ...
        File
            file =
            new File(this.plugin.getDataLocation(), "Schematics" + File.separator + res.getWorld() + File.separator + res.getName() + ".schematic");
        try {
    ...
    public boolean load(ClaimedResidence res) {
        ...
        ClaimedResidence r1 = this.plugin.getResidenceManager().getByLoc(new Location(bworld, or.getBlockX(), or.getBlockY(), or.getBlockZ()));
        ClaimedResidence r2 = this.plugin
            .getResidenceManager().getByLoc(new Location(bworld, or.getBlockX() + cc.getWidth() - 1, or.getBlockY() + cc.getHeight() - 1, or
                                                                                                                                              .getBlockZ()
                                                                                                                                          + cc.getLength()
                                                                                                                                          - 1));
        if (r1 == null || r2 == null) {
...
//	int totalBlocks = cc.getHeight() * cc.getLength() * cc.getWidth();
    ...
    public boolean rename(ClaimedResidence res, String newName) {
        ...
        File
            newFile =
            new File(this.plugin.getDataLocation(), "Schematics" + File.separator + res.getWorld() + File.separator + newName + ".schematic");
