...
public class PlayerGroup {
    ...
    public void updateGroup(String world, boolean force) {
        ...
        String group;
        if (Residence.getInstance().getPermissionManager().getPlayersGroups().containsKey(this.resPlayer.getPlayerName().toLowerCase())) {
            group = Residence.getInstance().getPermissionManager().getPlayersGroups().get(this.resPlayer.getPlayerName().toLowerCase());
            if (group != null) {
        ...
        group = Residence.getInstance().getPermissionManager().getPermissionsGroup(this.resPlayer.getPlayerName(), world);
    ...
    private PermissionGroup getPermissionGroup() {
        ...
        for (Entry<String, PermissionGroup> one : Residence.getInstance().getPermissionManager().getGroups().entrySet()) {
            if (player != null) {
                ...
                OfflinePlayer offlineP = Residence.getInstance().getOfflinePlayer(this.resPlayer.getPlayerName());
                if (offlineP != null) {
                    if (ResidenceVaultAdapter
                        .hasPermission(offlineP, "residence.group." + one.getKey(), Residence.getInstance().getConfigManager().getDefaultWorld())) {
                        group = one.getValue();
                    }
                }
            }
