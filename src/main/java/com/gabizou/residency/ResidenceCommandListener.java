...
import com.gabizou.residency.event.ResidenceCommandEvent;
...
public class ResidenceCommandListener extends Residence {
    ...
    private static cmd getCmdClass(String[] args) {
        ...
        try {
            Class<?> nmsClass;
            nmsClass = Class.forName("com.gabizou.residency.commands." + args[0].toLowerCase());
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equals("resload")) {
            if (!(sender instanceof Player) || sender instanceof Player && getPermissionManager().isResidenceAdmin(sender) && ((Player) sender)
                .hasPermission(
                    "residence.topadmin")) {
                try {
                    this.loadYml();
            ...
            if (!resadmin && player != null && this.resadminToggle.contains(player.getName())) {
